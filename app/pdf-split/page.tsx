"use client"
import React from 'react';
import PDFUploader from '@/components/pdf-splitter/PDFUploader';
import SplitList from '@/components/pdf-splitter/SplitList';
import ExportPanel from '@/components/pdf-splitter/ExportPanel';
import dynamic from 'next/dynamic';

// Dynamically import PDFPreviewer to avoid SSR issues with pdf.js
const PDFPreviewer = dynamic(() => import('@/components/pdf-splitter/PDFPreviewer'), { ssr: false });

const Page = () => {

    return (
        <div className="relative w-full text-white overflow-x-auto px-4">
            <div className="max-w-5xl mx-auto py-4">
                <h1 className="text-3xl font-bold text-center mb-4">PDF Splitt</h1>
                    <PDFUploader />
            </div>
            <div className="flex flex-col xl:flex-row gap-4 xl:gap-8 max-w-5xl mx-auto py-4">
                <div className="w-full xl:w-2/3 border-white/5 rounded-lg flex justify-center">
                    <PDFPreviewer />
                </div>
                <div className="w-full xl:w-1/3 flex flex-col gap-6">
                    <SplitList />
                    <ExportPanel />
                </div>
            </div>
        </div>
    );
};

export default Page;
